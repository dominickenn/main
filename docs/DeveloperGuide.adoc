...
** link:{repoURL}/src/main/java/seedu/address/logic/parser/OrganizerParser.java[`OrganizerParser`] is responsible for analyzing command words.
* Solution
** Modify the switch statement in link:{repoURL}/src/main/java/seedu/address/logic/parser/OrganizerParser.java[`OrganizerParser#parseCommand(String)`] such that both the proper command word and alias can be used to execute the same intended command.
...
|`* *`|user using the same machine as others |login into PrioriTask with my own account|manage my own tasks
